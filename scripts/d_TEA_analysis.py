...
'''
This module calculates the NPV for different pricing strategies.
'''
...
def NPVEstimate(NPVparams):
    ...
    for type in Type:
        ...
        output['NPVeachYear'] = np.round(np.divide(cashflow,
                discount_rate))
        filename = \
            'NPVeachYearBaseScenario_{}_{}_{}.csv'.format(existing_towers_MC,
                existing_towers_SC, type)
...
def ARPUvariations(NPVparams):
    '''
    Estimate the NPV of the given cost by varying the base case parameter values
    by -93% to +100%, to study the impact of revenue on the network feasibility
    using different infrastructure sharing strategies.
    '''
    ...
    for change in change:
        ...
        revenueupgradeSubscriber = upgradeSubscriber \
            * additionalSpending * ARPU5G
        revenueadditionalSubscriber = additionalSubscriber * ARPU5G
        totalrevenueperYear = revenueadditionalSubscriber \
            + revenueupgradeSubscriber
        ...
        for type in Type:
            ...
            filename = \
                'NPVeachYear_ARPU_{}_{}_{}_{}.csv'.format(existing_towers_MC,
                    existing_towers_SC, type, change)
