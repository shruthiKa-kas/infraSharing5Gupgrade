...
'''
This module takes input based on different network parameters, terrain details
and other parameters involved in rural 5G network deployment. This data helps 
us in studying the pathloss between the gNode B transmitter and user equipment
(UE). Next, this module considers the 3GPP RMa model to study the propagation
loss using the stochastic geometry with varying confidence intervals.
'''
...
def linkbudgetEsitimation(params):
    ...
    This function estimate the link budget between the gNode B and UE at various
    locations. The model uses the 3GPP NR link budget estimation for downlink (line of sight).
    '''
    ...
    '''
    DL losses: Other porpagation losses due to inteference, body loss, 
    foilage, weather, PRBs, sub-carrier power losses and other factors
    '''
    ...
    for d_UE_Tx in range(100, 10000, 300):
        ...
        for _ in range(20):
            ...
            PathLoss_PropagationModel = propgationLosses(
                hBS,
                hUT,
                fc_MHz,
                c,
            ...
            LinkBudget_SignalAtReceiver = Total_transmit_power_dBm \
                + antenna_gain_tx - cableLoss_tx + antenna_gain_rx \
                - cable_loss_rx - PathLoss_PropagationModel \
                - FoilageLoss_dB - BodyLoss_dB - InterferenceMargain_dB \
                - RainMargin_dB - slow_fading_margin_dB \
            ...
            Receiver_sensitivity_dBm = utnoisefigure + ThermalNoise_dB \
                + target_SINR
            SNRdB = LinkBudget_SignalAtReceiver \
                - Receiver_sensitivity_dBm
            SNR = math.pow(10, SNRdB / 10)
            channelThrouput = bandwidth_MHz * math.pow(10, 6) \
                * math.log(1 + SNR, 10)
            if abs(Receiver_sensitivity_dBm) \
                > abs(LinkBudget_SignalAtReceiver):
                y = 'pass'
                channelThrouput = bandwidth_MHz * math.pow(10, 6) \
                    * math.log(1 + SNR, 10)
            else:
            ...
            if fc_MHz == 700:
                Multiply = 4
            spectralefficiency = Multiply * channelThrouput \
                / (bandwidth_MHz * math.pow(10, 6))
            output.append({
                'iteration': _,
        ...
        filename = \
            'stochasticmodel_{}_{}_{}_{}.csv'.format(params['frequency_Mhz'
                ], params['bandwidth_Mhz'], params['carrier_spacing_KHz'
...
def propgationLosses(
    ...
    if type_coverage == 'LOS':
        ...
        mu = 0
        if d_UE_Tx < dBP:
            PLRMaLOSresults = pathlossLOScal(d3D, fc, h) + mu + z \
                * standarddeviation_rural
            PLRMaLOSresults = PLRMaLOSresults \
                * random.uniform(confidence_interval, 1)
        elif d_UE_Tx < 10000:
            PLRMaLOSresults = pathlossLOScal(dBP, fc, h) + 40 \
                * math.log10(d3D / dBP) + mu + z \
                * standarddeviation_rural
            PLRMaLOSresults = PLRMaLOSresults \
                * random.uniform(confidence_interval, 1)
...
def pathlossLOScal(d3D, fc, h):
    """
    Line of sight (LOS) path loss estimation as per 3GPP RMa model,
    with standard deviation 6 and mean 0
    
    """

    PLRMaLOS = 20 * math.log10(40 * math.pi * d3D * fc / 3000) \
        + min(0.03 * math.pow(h, 1.72), 10) * math.log10(d3D) \
        - min(0.044 * math.pow(h, 1.72), 14.77) + 0.002 * math.log10(h) \
        * d3D
    return PLRMaLOS
