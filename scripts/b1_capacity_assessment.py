...
'''
This module estimates the speed per cell as well as per user, and radius for a
hexagonal cell. This detail is used to estimate the number of cells required 
for upgrade.
'''
...
def capacity(params):
    '''
    Сalculator allows to calculate the maximum throughput of 5G NR network for
    user (depending on his mobile device UE) or cell. Approximately data transfer
    rate of 5G NR can be calculated using the formula: 3GPP specifications 
    TS 38.306 shares a formula for calculating the maximum data rate that a 
    UE/cell can support in downlink and uplink. 
    v(j)Layers - maximum number of MIMO layers ,3GPP 38.802:
    maximum 8 in DL, maximum 4 in UL
    nbeam = 1, minimum
    ...
    NbwPRB = float(math.floor(Bandwidth_MHz * 1000
                   / (FrequencySpacing_KHz * 12))) - 3
    ...
    '''
    3GPP 38.101 has specified maximum transmission bandwidth configuration for
    each UE channel and sub-carrier spacing provided in below table. The resource
    block number shown are after removing guard band from channel bandwidth and
    maximum bandwidth considered is 100 MHz
    '''

    GuardBand = float(0.5 * Bandwidth_MHz * 1000 - 0.5
                      * FrequencySpacing_KHz * 12 * NbwPRB - 0.5
    ...
    dataRates_Mbps = math.pow(10, -6) * v * Qjm * f * Rmax * NbwPRB \
        * 12 * (1 - OH) / Ts * nbeam
    usable_dataRates_Mbps = dataRates_Mbps * T
    print (OneRB_size_frequencyDomain_KHz,
           'OneRB_size_frequencyDomain_KHz')
    ...
    print (Mode, 'Usable Data Rates (Mbps)', usable_dataRates_Mbps)
     ...
     ''' 
    To determine a cell’s overall practical capacity for broadband, and thus to
    evaluate the real capability of any proposed network that leverages wireless
    technology, one must consider the average of the experience among all users
    near and far. This is often only 15–25 percent of the theoretical peak for a 
    single user. When overheads are considered, the usable capacity will 
    typically be less than 75 percent of this value. It therefore is not unusual
    for the actual throughput capacity to be only roughly 15 percent of its peak
    data connection rate – although the latter is the speed that is usually
    promoted.
    '''
    ...
    output = []
    filename = \
        'capacityCalculator_{}_{}_{}.csv'.format(params['Bandwidth_MHz'
            ], params['Frequency_MHz'], params['FrequencySpacing_KHz'])
...
def maximumAllowablePathLoss(params):
    ...
    Bandwidth_MHz = float(params['Bandwidth_MHz'])
    FrequencySpacing_KHz = float(params['FrequencySpacing_KHz'])
    ThermalNoise_dB = -174 + 10 * math.log10(Bandwidth_MHz
            * math.pow(10, 6))
    NbwPRB = float(math.floor(Bandwidth_MHz * 1000
                   / (FrequencySpacing_KHz * 12))) - 3
    if FrequencySpacing_KHz == 15:
    ...
    Reference_singal_power_dBm = Max_Tx_Power - 10 * math.log10(NbwPRB
            * 12)
    Total_transmit_power_dBm = Max_Tx_Power + 10 \
        * math.log10(No_Antennas)
    ...
    if TypeofCoverageLocation == 'indoor':
        if fc_MHz == 3800:
            ...
            PenetrationLoss_indoor = PenetrationLoss_indoorperdoor \
                * walls
    ...
    Receiver_sensitivity_dBm = -utnoiseFigure + ThermalNoise_dB \
        - TargetSINR
    df1 = []
    ...
    max_allowable_prop_pathLoss = Max_Tx_Power \
        - Receiver_sensitivity_dBm + AntennaGain_Tx - CableLoss_Tx \
        + AntennaGain_Rx - CableLoss_Rx - FoilageLoss_dB - BodyLoss_dB \
        - InterferenceMargain_dB - RainMargin_dB - SlowFadingMargin_dB \
        - PenetrationLoss_indoor - AttentuationLoss_indoor
    ...
    constant = 10 * math.log10(lambda_m * lambda_m / (16 * math.pi
                               * math.pi))
    if fc_MHz == 700:
        cell_radius_m = math.pow(10, (max_allowable_prop_pathLoss - 5
                                 - 20 * math.log10(fc_MHz)) / 22)
        print (cell_radius_m, 'cell_radius_m method 1')
        isd = 2 * cell_radius_m
    elif fc_MHz == 3800:
        cell_radius_m = math.pow(10, (max_allowable_prop_pathLoss
                                 / 1.05 + constant) / 22)
    ...
    output = []
    filename = \
        'cell_radius_Calculator_{}_{}_{}.csv'.format(params['Bandwidth_MHz'
            ], params['Frequency_MHz'], params['FrequencySpacing_KHz'])
