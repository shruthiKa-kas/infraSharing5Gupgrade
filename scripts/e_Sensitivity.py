...
This module estimates the sensitivity of different parameters on the feasibility
of the different infrastructure sharing strategies.
'''

params = {
    'Core_cost_percentage': 0.10,
    'backhaul_MC': 10000,
    'backhaul_SC_m': 5,
    'tower': 1000,
    'OPEX_rental': 0.10,
...
def NPVEstimate(
    ...
    Subscriber_growth_factor = \
        float(sensitivityparams['Subscriber_growth_factor'])
    ...
    Upgrade_infrastructure_factor = \
        float(sensitivityparams['Upgrade_infrastructure_factor'])
    Demand_factor = float(sensitivityparams['Demand_factor'])
    
    '''
    maincalculations
    
    These equations estimate NPV for each change in the input parameter.
    '''
    ...
    existing_towers_SC = Upgrade_infrastructure_factor \
        * int(Number_of_SC_deployed)
    existing_towers_MC = Upgrade_infrastructure_factor \
        * int(Number_of_MC_upgrade)

    CAPEX_small = cost_RAN_small_USD * CAPEX_factor
    CAPEX_macro = CAPEX_factor * (carrier_aggregation + control_units
                                  + iO_fronthaul + remote_radio_units
                                  + processing)
    CAPEX_rateofChange = np.power(1 + CAPEX_rateofChange, year - 2023)
    Core = CAPEX_factor * (core_cost_percentage * (existing_towers_SC
                           * cost_RAN_small_USD + existing_towers_MC
                           * (carrier_aggregation + control_units
                           + iO_fronthaul + remote_radio_units
                           + processing)))
    ...
    OPEX_macro = OPEX_factor * (OPEX_rental * CAPEX_macro
                                + power_supply) * 10
    year = np.array([
        2023,
        2024,
    ...
    Badloans_rateofChange = np.power(1 + Badloans_rateofChange
            * Debt_payment_factor, year - 2023)
    ...
    additionalSubscriber = 0.20 * population
    totalSubscriber = upgradeSubscriber + additionalSubscriber

    subscriberGrowth = np.power(1 + subscriberGrowth
                                * Subscriber_growth_factor, year - 2023)
    ...
    revenueupgradeSubscriber = upgradeSubscriber * additionalSpending \
        * ARPU5G * Demand_factor
    revenueadditionalSubscriber = additionalSubscriber * ARPU5G \
        * Demand_factor
    totalrevenueperYear = revenueadditionalSubscriber \
        + revenueupgradeSubscriber
    ...
    def NPVCase(type):
        ...
        CAPEX = (CAPEX_small * existing_towers_SC * r
                 + siteRental_rural_small * t + Spectrum_factor
                 * bandwidth_3800MHz * cost_spectrum_3800_USD
                 * population * s + Backhaul_factor * backhaul_SC_m
                 * 1000 * b + CAPEX_macro * existing_towers_MC * r
                 + siteRental_rural_macro * t + Core * c
                 + Spectrum_factor * bandwidth_700Mhz
                 * cost_spectrum_700_USD * population * s
                 + Backhaul_factor * backhaul_MC * b) / 10 \
            * CAPEX_rateofChange
        ...
        output['NPV'] = np.round(np.sum(np.divide(cashflow,
                                 discount_rate)))
        return output
