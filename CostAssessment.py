...
def costEstimate(params):
    ...
    """
    run_only_once = float(params['run_only_once'])
    if run_only_once == 0:
        df = pd.DataFrame(params, index=[0])
        filename = "cost_input_base.csv"
        if not os.path.exists('Data/Cost'):
                os.mkdir('Data/Cost')
        my_path = os.path.join('Data/Cost', filename)
        df.to_csv(my_path, index=False, header=True)
        
#     [Number_of_MC_upgrade, Number_of_SC_deployed] = TEAmodel.TowerRequired(paramsTower)
# #     filename = "Upgrade.csv"
# #     my_path = os.path.join('results/UpgradeRequirements', filename)
# #     # df = pd.read_csv(my_path) 
# #     # with open(my_path, mode='r') as infile:
...
# #     tower_col = {}
# #     for row in reader:
# #         tower_col[row[0]]= { 'Number_of_SC_upgrade':row[2]}
# #         # tower_col["names"].append({"existing_towers_SC"}: row[0], "existing_towers_MC": row[1}])
# #         # tower_col['existing_towers_MC'].append(record['existing_towers_MC'])
# #     print(tower_col)
# #     # existing_towers_MC = reader.get("existing_towers_MC")
# #     # existing_towers_SC = reader.get("existing_towers_SC")
    ...
    existing_towers_SC = float(params['existing_towers_SC'])
    existing_towers_MC = float(params['existing_towers_MC'])
...
#             debt,opex_small = initialiseparameters(params)
    ...
    def eachCasecost(type):    
        ...
        CAPEX = (((CAPEX_small)*existing_towers_SC*r + siteRental_rural_small*t +  bandwidth_3800MHz*cost_spectrum_3800_USD*population*s + backhaul_SC_m*1000*b) + \
                    ((CAPEX_macro)*existing_towers_MC*r+ siteRental_rural_macro*t +Core*c + bandwidth_700Mhz*cost_spectrum_700_USD*population*s + backhaul_MC*b))/10*CAPEX_rateofChange
        OPEX =  ((OPEX_small)*existing_towers_SC*r + (OPEX_macro)*existing_towers_MC*r)/10*OPEX_rateofchange
        TCO = CAPEX + OPEX
        # print(TCO_NS, 'TCO_NS')
        Badloans = TCO_NS/10*Badloans_rateofChange
        TCO_plus_badloans = TCO + Badloans

        arr = np.stack((strategy,year,CAPEX, OPEX,Badloans,TCO_plus_badloans), axis = 1)
        return arr
        #return strategy,TCO
    ...
    # #"Solo", "Passive", "Active", "NHN5G"    
        ...
        filename = "cost_{}_{}_{}.csv".format(
            params['existing_towers_MC'], 
            params['existing_towers_SC'],
            type,
        )
